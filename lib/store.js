...
// Update editable profile fields. Only the keys present in `fields` are updated.
export async function updateUserProfile(id, fields) {
  ...
  if (fields.bodyWeight !== undefined) patch.body_weight = fields.bodyWeight;
...
export async function setUserAvatar(id, avatar) {
  const { error } = await getSupabase().from("users").update({ avatar }).eq("id", id);
  if (error) throw error;
}

export async function setUserHealthPlan(id, plan) {
  const { error } = await getSupabase().from("users").update({ health_plan: plan }).eq("id", id);
  if (error) throw error;
}

// ---------- Health-plan history (for lab trends across reports) ----------
export async function insertHealthPlanHistory({ userId, plan, labs, reportDate }) {
  const { error } = await getSupabase().from("health_plan_history").insert({
    user_id: userId,
    plan,
    labs: labs || null,
...
export async function getHealthPlanHistory(userId, limit = 24) {
  const { data, error } = await getSupabase()
    .from("health_plan_history")
    .select("id,plan,labs,report_date,created_at")
    .eq("user_id", userId)
...
export async function createVitalLog({ userId, type, date, time, value, systolic, diastolic, context, note }) {
  const row = {
    user_id: userId,
    type,
    date,
...
export async function getVitalLogs(userId, { type, limit = 60 } = {}) {
  let q = getSupabase()
    .from("vital_logs")
    .select("*")
    .eq("user_id", userId)
...
export async function deleteVitalLog(userId, id) {
  const { error } = await getSupabase()
    .from("vital_logs")
    .delete()
    .eq("user_id", userId)
...
// Delete by endpoint regardless of user (used when a push gets 404/410 = gone).
...
// All subscriptions across users (for the scheduler), grouped by user.
export async function getAllPushSubscriptions() {
  ...
  if (error) throw error;
  const byUser = {};
  for (const r of data || []) (byUser[r.user_id] ||= []).push(rowToSub(r));
  return byUser;
}
...
// Record that a reminder push was sent; returns true if newly recorded,
// false if it was already sent today (so the caller skips re-sending).
export async function claimPushTag(userId, tag, date) {
  const { error } = await getSupabase()
    .from("push_logs")
    .insert({ user_id: userId, tag, date });
  if (error) {
